...
def center_actor(input_dim_list, cnn_kernel_size):
    ...
    theOmega = tf.reshape(theOmega, input_dim_list[1])
    ...
    #theOmega process
    Omega = layers.Dense(2, activation='relu')(theOmega)
    #Omega = tf.squeeze(Omega, axis=-1)

    bandwidth_dense = layers.concatenate([buffer_state, pos, Omega], axis=-1)  # shape = (None, 4, 4)
    ...
    afterFuse = layers.Dense(9, activation='relu')(bandwidth_dense)
